...
var passport = require('passport-strategy')
  ...
  , HttpsAgent = require('node:https').Agent
  , utils = require('./utils')
 ...
 *   - `rejectUnauthorized` when set and tokenURL starts with https, allows to enable/disable server validation
 *   - `ca`                 A Base64 encoded Buffer containing the CA certificates for server validation
 *
 * Examples:
 *
...
function OAuth2Strategy(options, verify) {
  ...
  // setup agent for validating connection
  if (options.tokenURL.match(/^https:/)) {
    this._oauth2.setAgent(new HttpsAgent({
      rejectUnauthorized: options.rejectUnauthorized === undefined ? true : options.rejectUnauthorized,
      ca: options.ca === undefined ? undefined : Buffer.from(options.ca, 'base64')
    }));
  }
...
OAuth2Strategy.prototype.authenticate = function(req, options) {
  ...
  if ((req.query && req.query.code) || (req.body && req.body.code)) {
    ...
    if (this._pkceMethod) {
      verifier = Buffer.from(crypto.pseudoRandomBytes(32)).toString('base64url');
      switch (this._pkceMethod) {
      case 'plain':
        challenge = verifier;
        break;
      case 'S256':
        challenge = Buffer.from(crypto.createHash('sha256').update(verifier).digest()).toString('base64url');
        break;
      default:
